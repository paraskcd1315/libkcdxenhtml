...
var drawer = {
	...
	animateApp: function (el) {
		if (
			(el.target.className === 'drawerApp' ||
				el.target.className === 'hsApp' ||
				el.target.className === 'musicButtons') &&
			!drawer.movedWhilePressing
		) {
	...
	displayApps: function (filteredApps, page) {
		...
		mainDiv.addEventListener(
			'touchmove',
			() => (drawer.movedWhilePressing = true),
			false
		);
		touchhold.init({
			time: 400,
		...
		for (let i = 0; i < this.appContainer.length; i++) {
			...
			let menuItem = {
				...
				callback: function () {
					drawer.invokeMenu = false;
					if (
						!localstore[appContainerStuff.id] ||
						appContainerStuff.limit !== localstore[appContainerStuff.id].length
					) {
						let appContainer = document.getElementsByClassName(
							appContainerStuff.id
						)[0];
